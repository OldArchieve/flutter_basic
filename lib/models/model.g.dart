...
//  - start typing ex:Image.select()... (add a few filters with fluent methods)...(add orderBy/orderBydesc if you want)...
...
// BEGIN TABLES
// Image TABLE
class TableImage extends SqfEntityTableBase {
  TableImage() {
  ...
  static SqfEntityTableBase get getInstance {
    return _instance = _instance ?? TableImage();
  }
}
...
class BasicModel extends SqfEntityModelProvider {
  BasicModel() {
    databaseName = basicModel.databaseName;
    databaseTables = [
      TableImage.getInstance,
      TableFolder.getInstance,
      TableUser.getInstance,
    ];

    bundledDatabasePath = basicModel
...
class Image {
  ...
  Image.withFields(this.title, this.imageUrl, this.createdAt, this.updatedAt,
      this.isActive, this.folderId, this.isDeleted) {
    _setDefaultValues();
  }
  Image.withId(this.id, this.title, this.imageUrl, this.createdAt,
      this.updatedAt, this.isActive, this.folderId, this.isDeleted) {
    _setDefaultValues();
  }
  Image.fromMap(Map<String, dynamic> o) {
    id = o['id'] as int;
...
// RELATIONSHIPS (Image)
  ...
  Future<Folder> getFolder([VoidCallback Function(Folder o) folder]) async {
  ...
  // END RELATIONSHIPS (Image)

  static const bool _softDeleteActivated = true;
  ImageManager __mnImage;

  ImageManager get _mnImage {
    return __mnImage = __mnImage ?? ImageManager();
  }
  ...
  static Future<List<Image>> fromWebUrl(String url) async {
    try {
      final response = await http.get(url);
      return await fromJson(response.body);
    } catch (e) {
      print('SQFENTITY ERROR Image.fromWebUrl: ErrorMessage: ${e.toString()}');
      return null;
    }
  }

  static Future<List<Image>> fromJson(String jsonBody) async {
    final Iterable list = await json.decode(jsonBody) as Iterable;
    var objList = <Image>[];
    try {
      objList = list
          .map((image) => Image.fromMap(image as Map<String, dynamic>))
          .toList();
    } catch (e) {
      print('SQFENTITY ERROR Image.fromJson: ErrorMessage: ${e.toString()}');
    }
    return objList;
  }
  ...
  /*
    ...
    static Future<List<Image>> fromObjectList(Future<List<dynamic>> o) async {
      final data = await o;
      return await Image.fromMapList(data);
    } 
    */

  static Future<List<Image>> fromMapList(List<dynamic> data,
      {bool preload = false, List<String> preloadFields}) async {
    final List<Image> objList = <Image>[];
    for (final map in data) {
      final obj = Image.fromMap(map as Map<String, dynamic>);

      // RELATIONSHIPS PRELOAD
      if (preload) {
        if (preloadFields.isEmpty || preloadFields.contains('plFolder')) {
          obj.plFolder = await obj.getFolder();
        }
  ...
  /// returns Image by ID if exist, otherwise returns null
  /// <param name='id'>Primary Key Value</param>
  /// <returns>returns Image if exist, otherwise returns null
  Future<Image> getById(int id) async {
    if (id == null) {
      return null;
    }
    Image obj;
    final data = await _mnImage.getById(id);
    if (data.length != 0) {
      obj = Image.fromMap(data[0] as Map<String, dynamic>);
    } else {
      obj = null;
    }
    return obj;
  ...
  /// saveAs Image. Returns a new Primary Key value of Image

  /// <returns>Returns a new Primary Key value of Image
  ...
  /// saveAll method saves the sent List<Image> as a bulk in one transaction
  /// Returns a <List<BoolResult>>
  Future<List<BoolResult>> saveAll(List<Image> images) async {
    final results = _mnImage.saveAll(
        'INSERT OR REPLACE INTO image (id,  title, imageUrl, createdAt, updatedAt, isActive, folderId,isDeleted)  VALUES (?,?,?,?,?,?,?,?)',
        images);
    return results;
  }
    ...
    try {
      if (await _mnImage.rawInsert(
        ...
        saveResult = BoolResult(
            success: true, successMessage: 'Image id=$id updated successfuly');
      } else {
        saveResult = BoolResult(
            success: false, errorMessage: 'Image id=$id did not update');
      }
      return id;
    } catch (e) {
      saveResult = BoolResult(
          success: false,
          errorMessage: 'Image Save failed. Error: ${e.toString()}');
      return 0;
    }
  }

  /// inserts or replaces the sent List<<Image>> as a bulk in one transaction.
  /// upsertAll() method is faster then saveAll() method. upsertAll() should be used when you are sure that the primary key is greater than zero
  /// Returns a <List<BoolResult>>
  Future<List<BoolResult>> upsertAll(List<Image> images) async {
    final results = await _mnImage.rawInsertAll(
        'INSERT OR REPLACE INTO image (id,  title, imageUrl, createdAt, updatedAt, isActive, folderId,isDeleted)  VALUES (?,?,?,?,?,?,?,?)',
        images);
    return results;
  }
  ...
  Future<BoolResult> delete([bool hardDelete = false]) async {
    print('SQFENTITIY: delete Image invoked (id=$id)');
    if (!_softDeleteActivated || hardDelete || isDeleted) {
      return _mnImage
          .delete(QueryParams(whereString: 'id=?', whereArguments: [id]));
    } else {
      return _mnImage.updateBatch(
  ...
  Future<BoolResult> recover([bool recoverChilds = true]) async {
    print('SQFENTITIY: recover Image invoked (id=$id)');
    {
      return _mnImage.updateBatch(
          QueryParams(whereString: 'id=?', whereArguments: [id]),
          {'isDeleted': 0});
  ...
  //private ImageFilterBuilder _Select;
  ImageFilterBuilder select({List<String> columnsToSelect, bool getIsDeleted}) {
    return ImageFilterBuilder(this)
      .._getIsDeleted = getIsDeleted == true
      ..qparams.selectColumns = columnsToSelect;
  }

  ImageFilterBuilder distinct(
...
// region ImageField
class ImageField extends SearchCriteria {
  ImageField(this.imageFB) {
    param = DbParameter();
  }
  DbParameter param;
  String _waitingNot = '';
  ImageFilterBuilder imageFB;
  ...
  ImageFilterBuilder equals(dynamic pValue) {
    param.expression = '=';
    imageFB._addedBlocks = _waitingNot == ''
        ? setCriteria(pValue, imageFB.parameters, param, SqlSyntax.EQuals,
            imageFB._addedBlocks)
        : setCriteria(pValue, imageFB.parameters, param, SqlSyntax.NotEQuals,
            imageFB._addedBlocks);
    _waitingNot = '';
    imageFB._addedBlocks.needEndBlock[imageFB._blockIndex] =
        imageFB._addedBlocks.retVal;
    return imageFB;
  }

  ImageFilterBuilder equalsOrNull(dynamic pValue) {
    param.expression = '=';
    imageFB._addedBlocks = _waitingNot == ''
        ? setCriteria(pValue, imageFB.parameters, param, SqlSyntax.EQualsOrNull,
            imageFB._addedBlocks)
        : setCriteria(pValue, imageFB.parameters, param,
            SqlSyntax.NotEQualsOrNull, imageFB._addedBlocks);
    _waitingNot = '';
    imageFB._addedBlocks.needEndBlock[imageFB._blockIndex] =
        imageFB._addedBlocks.retVal;
    return imageFB;
  ...
  ImageFilterBuilder isNull() {
    imageFB._addedBlocks = setCriteria(
        0,
        imageFB.parameters,
        param,
  ...
  ImageFilterBuilder contains(dynamic pValue) {
    if (pValue != null) {
      imageFB._addedBlocks = setCriteria(
          '%${pValue.toString()}%',
          imageFB.parameters,
  ...
  ImageFilterBuilder startsWith(dynamic pValue) {
    if (pValue != null) {
      imageFB._addedBlocks = setCriteria(
          '${pValue.toString()}%',
          imageFB.parameters,
  ...
  ImageFilterBuilder endsWith(dynamic pValue) {
    if (pValue != null) {
      imageFB._addedBlocks = setCriteria(
          '%${pValue.toString()}',
          imageFB.parameters,
  ...
  ImageFilterBuilder between(dynamic pFirst, dynamic pLast) {
    if (pFirst != null && pLast != null) {
      ...
      if (_waitingNot != '') {
        imageFB._addedBlocks = setCriteria(pFirst, imageFB.parameters, param,
            SqlSyntax.LessThan, imageFB._addedBlocks);
      } else {
        imageFB._addedBlocks = setCriteria(pFirst, imageFB.parameters, param,
            SqlSyntax.GreaterThanOrEquals, imageFB._addedBlocks);
      }
    } else if (pLast != null) {
      if (_waitingNot != '') {
        imageFB._addedBlocks = setCriteria(pLast, imageFB.parameters, param,
            SqlSyntax.GreaterThan, imageFB._addedBlocks);
      } else {
        imageFB._addedBlocks = setCriteria(pLast, imageFB.parameters, param,
            SqlSyntax.LessThanOrEquals, imageFB._addedBlocks);
      }
    }
    _waitingNot = '';
    imageFB._addedBlocks.needEndBlock[imageFB._blockIndex] =
  ...
  ImageFilterBuilder greaterThan(dynamic pValue) {
    param.expression = '>';
    imageFB._addedBlocks = _waitingNot == ''
        ? setCriteria(pValue, imageFB.parameters, param, SqlSyntax.GreaterThan,
            imageFB._addedBlocks)
        : setCriteria(pValue, imageFB.parameters, param,
            SqlSyntax.LessThanOrEquals, imageFB._addedBlocks);
    _waitingNot = '';
  ...
  ImageFilterBuilder lessThan(dynamic pValue) {
    param.expression = '<';
    imageFB._addedBlocks = _waitingNot == ''
        ? setCriteria(pValue, imageFB.parameters, param, SqlSyntax.LessThan,
            imageFB._addedBlocks)
        : setCriteria(pValue, imageFB.parameters, param,
            SqlSyntax.GreaterThanOrEquals, imageFB._addedBlocks);
    _waitingNot = '';
  ...
  ImageFilterBuilder greaterThanOrEquals(dynamic pValue) {
    param.expression = '>=';
    imageFB._addedBlocks = _waitingNot == ''
        ? setCriteria(pValue, imageFB.parameters, param,
            SqlSyntax.GreaterThanOrEquals, imageFB._addedBlocks)
  ...
  ImageFilterBuilder lessThanOrEquals(dynamic pValue) {
    param.expression = '<=';
    imageFB._addedBlocks = _waitingNot == ''
        ? setCriteria(pValue, imageFB.parameters, param,
            SqlSyntax.LessThanOrEquals, imageFB._addedBlocks)
  ...
  ImageFilterBuilder inValues(dynamic pValue) {
    imageFB._addedBlocks = setCriteria(
        pValue,
        imageFB.parameters,
        param,
...
// endregion ImageField

// region ImageFilterBuilder
class ImageFilterBuilder extends SearchCriteria {
  ImageFilterBuilder(Image obj) {
    whereString = '';
    qparams = QueryParams();
  ...
  /// open parentheses
  ImageFilterBuilder get startBlock {
  ...
  ImageFilterBuilder where(String whereCriteria) {
  ...
  ImageFilterBuilder page(int page, int pagesize) {
  ...
  /// int count = LIMIT
  ImageFilterBuilder top(int count) {
    if (count > 0) {
      _pagesize = count;
    }
    return this;
  ...
  /// close parentheses
  ImageFilterBuilder get endBlock {
  ...
  ImageFilterBuilder orderBy(dynamic argFields) {
    if (argFields != null) {
  ...
  ImageFilterBuilder orderByDesc(dynamic argFields) {
    if (argFields != null) {
      if (argFields is String) {
  ...
  ///
  /// Example 2: argFields = ['field1', 'field2']
  ImageFilterBuilder groupBy(dynamic argFields) {
    if (argFields != null) {
      if (argFields is String) {
  ...
  ImageField setField(ImageField field, String colName, DbType dbtype) {
  ...
  /// Deletes List<Image> bulk by query
  ///
  /// <returns>BoolResult res.success=Deleted, not res.success=Can not deleted
  Future<BoolResult> delete([bool hardDelete = false]) async {
    _buildParameters();
    var r = BoolResult();
    if (Image._softDeleteActivated && !hardDelete) {
      r = await _obj._mnImage.updateBatch(qparams, {'isDeleted': 1});
    } else {
      r = await _obj._mnImage.delete(qparams);
    }
    return r;
  }
  ...
  Future<BoolResult> recover() async {
    _getIsDeleted = true;
    _buildParameters();
    print('SQFENTITIY: recover Image bulk invoked');
    return _obj._mnImage.updateBatch(qparams, {'isDeleted': 0});
  }
  ...
  Future<BoolResult> update(Map<String, dynamic> values) {
    ...
    if (qparams.limit > 0 || qparams.offset > 0) {
      qparams.whereString =
          'id IN (SELECT id from image ${qparams.whereString.isNotEmpty ? 'WHERE ${qparams.whereString}' : ''}${qparams.limit > 0 ? ' LIMIT ${qparams.limit}' : ''}${qparams.offset > 0 ? ' OFFSET ${qparams.offset}' : ''})';
    }
    return _obj._mnImage.updateBatch(qparams, values);
  }

  /// This method always returns ImageObj if exist, otherwise returns null
  ///
  ...
  ///
  /// <returns>List<Image>
  Future<Image> toSingle(
      {bool preload = false, List<String> preloadFields}) async {
    _pagesize = 1;
    _buildParameters();
    final objFuture = _obj._mnImage.toList(qparams);
  ...
  ///
  /// <returns>int
  Future<int> toCount([VoidCallback Function(int c) imageCount]) async {
    _buildParameters();
    qparams.selectColumns = ['COUNT(1) AS CNT'];
    final imagesFuture = await _obj._mnImage.toList(qparams);
    final int count = imagesFuture[0]['CNT'] as int;
    if (imageCount != null) {
      imageCount(count);
    }
    return count;
  ...
  /// This method always returns List<Image>.
  ///
  /// Set preload to true if you want to load all fields related to child or parent
  ///
  /// You can send certain field names with preloadFields parameter for preloading. For ex: toList(preload:true, preloadFields:['plField1','plField2'... etc])
  ///
  /// <returns>List<Image>
  Future<List<Image>> toList(
      {bool preload = false, List<String> preloadFields}) async {
    final data = await toMapList();
    final List<Image> imagesData =
        await Image.fromMapList(data, preload: preload);
    return imagesData;
  }
  ...
  Future<List<dynamic>> toMapList() async {
    _buildParameters();
    return await _obj._mnImage.toList(qparams);
  }

  /// Returns List<DropdownMenuItem<Image>>
  Future<List<DropdownMenuItem<Image>>> toDropDownMenu(String displayTextColumn,
      [VoidCallback Function(List<DropdownMenuItem<Image>> o)
          dropDownMenu]) async {
    _buildParameters();
    final imagesFuture = _obj._mnImage.toList(qparams);

    final data = await imagesFuture;
    final int count = data.length;
    final List<DropdownMenuItem<Image>> items = []..add(DropdownMenuItem(
        value: Image(),
    ...
    for (int i = 0; i < count; i++) {
      items.add(
        DropdownMenuItem(
          value: Image.fromMap(data[i] as Map<String, dynamic>),
  ...
  Future<List<int>> toListPrimaryKey([bool buildParameters = true]) async {
    ...
    final idFuture = await _obj._mnImage.toList(qparams);
  ...
  Future<List<dynamic>> toListObject(
    ...
    final objectFuture = _obj._mnImage.toList(qparams);
  ...
  ///
  /// Sample usage: await Image.select(columnsToSelect: ['columnName']).toListString()
...
// endregion ImageFields

//region ImageManager
class ImageManager extends SqfEntityProvider {
  ImageManager() : super(BasicModel(), tableName: _tableName, colId: _colId);
  static String _tableName = 'image';
...
// COLLECTIONS (Folder)
  ...
  /// get Image(s) filtered by folderId=id
  ImageFilterBuilder getImages(
      {List<String> columnsToSelect, bool getIsDeleted}) {
    return Image()
...
// COLLECTIONS (Folder)
    if (!forQuery) {
      map['Images'] = await getImages().toMapList();
    }
// END COLLECTIONS (Folder)
  ...
  static Future<List<Folder>> fromMapList(List<dynamic> data,
    ...
    for (final map in data) {
      ...
      // RELATIONSHIPS PRELOAD
      if (preload) {
        if (preloadFields.isEmpty || preloadFields.contains('plImages')) {
          obj.plImages = await obj.getImages().toList();
        }
      } // END RELATIONSHIPS PRELOAD
  ...
  Future<BoolResult> delete([bool hardDelete = false]) async {
    ...
    {
      result = await Image().select().folderId.equals(id).delete(hardDelete);
    }
    if (!result.success) {
      return result;
    }
...
class FolderFilterBuilder extends SearchCriteria {
  ...
  Future<BoolResult> delete([bool hardDelete = false]) async {
    _buildParameters();
    var r = BoolResult();
    final imageByfolderIdidList = await toListPrimaryKey(false);
    await Image()
        .select()
        .folderId
        .inValues(imageByfolderIdidList)
        .delete(hardDelete);
...
class FolderManager extends SqfEntityProvider {
  FolderManager() : super(BasicModel(), tableName: _tableName, colId: _colId);
...
class User {
  ...
  Future<List<BoolResult>> saveAll(List<User> users) async {
    final results = _mnUser.saveAll(
        'INSERT OR REPLACE INTO user (id,  firstName, lastName, email, password, profileUrl, isActive, createdAt, updatedAt,isDeleted)  VALUES (?,?,?,?,?,?,?,?,?,?)',
        users);
    return results;
  }
...
class UserManager extends SqfEntityProvider {
  UserManager() : super(BasicModel(), tableName: _tableName, colId: _colId);
  static String _tableName = 'user';
  static String _colId = 'id';
}

//endregion UserManager
class BasicModelSequenceManager extends SqfEntityProvider {
  BasicModelSequenceManager() : super(BasicModel());
}
// END OF ENTITIES
